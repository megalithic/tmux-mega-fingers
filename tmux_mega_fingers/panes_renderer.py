...
class PanesRenderer:
    ...
    def __init__(self, ui: UI, panes: List[Pane]):
        self.ui = ui
        self.panes = panes
        # space toggles copy (secondary) mode for the next selection, like upstream.
        self.secondary_mode = False
        # Fork: a shifted hint letter (e.g. `A` vs `a`) selects the copy action
        # for that one selection, regardless of the space-toggle mode.
    ...
    def loop(self) -> None:
        ...
        while True:
            ...
            if user_input:
                ...
                if chosen_mark:
                    if self.secondary_mode or self._shift_select:
    ...
    def _handle_user_input(self, user_input: str) -> str:
        ...
        if char == 127:
            ...
            c = chr(char)
            # Fork: shifted letter (A-Z) -> copy action for this selection;
            # match case-insensitively against the lowercase hint.
